...
// ✅ Import SEM @/ (mais seguro pra você)
...
// ✅ IPVA RJ (você passou os links certos)
const LINKS_IPVA = {
  ipva_sefaz: "https://portal.fazenda.rj.gov.br/ipva/",
  detran_servicos: "https://www.detran.rj.gov.br/todos-os-servicos/servicos-drv.html",
};

// ✅ Telefones úteis (com bombeiros + DEAT)
...
export default function UtilidadesClient() {
  ...
  // ✅ aplica querystring na primeira carga e quando URL mudar
  useEffect(() => {
    if (tabParam === "ipva" || tabParam === "telefones" || tabParam === "iptu") {
      setTab(tabParam);
    } else {
      setTab("iptu");
    }
  ...
  function setUrl(nextTab, nextCidade) {
    const params = new URLSearchParams();
    if (nextTab) params.set("tab", nextTab);
    if (nextCidade) params.set("cidade", nextCidade);
    router.push(`/utilidades?${params.toString()}`);
  }
      ...
      <section className="max-w-6xl mx-auto px-4 pt-8">
        ...
        <div className="mt-6 bg-white rounded-3xl border border-slate-200 shadow-sm p-2">
          <div className="grid grid-cols-3 gap-2 text-sm">
            <button
              type="button"
              onClick={() => trocarTab("iptu")}
              className={`rounded-2xl px-3 py-2 font-semibold transition ${
                tab === "iptu" ? "bg-slate-900 text-white" : "bg-slate-50 text-slate-700 hover:bg-slate-100"
              }`}
            >
              IPTU (Cidades)
            </button>
            ...
            <button
              type="button"
              onClick={() => trocarTab("ipva")}
              className={`rounded-2xl px-3 py-2 font-semibold transition ${
                tab === "ipva" ? "bg-slate-900 text-white" : "bg-slate-50 text-slate-700 hover:bg-slate-100"
              }`}
            >
              IPVA (RJ)
            </button>
            ...
            <button
              type="button"
              onClick={() => trocarTab("telefones")}
              className={`rounded-2xl px-3 py-2 font-semibold transition ${
                tab === "telefones" ? "bg-slate-900 text-white" : "bg-slate-50 text-slate-700 hover:bg-slate-100"
              }`}
            >
              Telefones úteis
            </button>
      ...
      <section className="max-w-6xl mx-auto px-4 mt-6">
        {tab === "iptu" && (
          <div className="grid gap-4 lg:grid-cols-[1fr,1.2fr]">
            ...
            <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-5">
              ...
              <p className="mt-1 text-xs text-slate-600">Escolha a cidade e abra o portal do IPTU / Prefeitura.</p>

              <label className="block mt-4 text-[11px] font-semibold text-slate-600">Selecione a cidade</label>
            ...
            <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-5">
              ...
              {!cidade && <p className="mt-2 text-sm text-slate-600">Clique em uma cidade para mostrar os links.</p>}

              {cidade && !linksCidade && (
                <p className="mt-2 text-sm text-slate-600">Ainda não há links cadastrados para esta cidade.</p>
              )}
              ...
              <p className="mt-4 text-[11px] text-slate-500">
                Alguns portais podem exigir CPF/CNPJ do contribuinte e abrir em nova aba.
              </p>
            </div>
          </div>
        )}
        ...
        {tab === "ipva" && (
          <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-5">
            ...
            <p className="mt-2 text-sm text-slate-600">Links oficiais para consulta e serviços relacionados.</p>

            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              <a
                href={LINKS_IPVA.ipva_sefaz}
                target="_blank"
                rel="noopener noreferrer"
                className="block rounded-2xl border border-slate-200 bg-slate-50 p-4 hover:bg-slate-100 transition"
              >
                <p className="text-sm font-extrabold text-slate-900">SEFAZ RJ — IPVA</p>
                <p className="text-[11px] text-slate-600 break-all">{LINKS_IPVA.ipva_sefaz}</p>
              </a>
        ...
        {tab === "telefones" && (
          <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-5">
            ...
            <p className="mt-2 text-sm text-slate-600">Lista básica (podemos expandir por cidade depois).</p>
